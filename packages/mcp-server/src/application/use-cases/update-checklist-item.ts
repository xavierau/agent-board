import { createChecklistItemUpdatedEvent } from '../../domain/events/checklist-event-factories.js';
...
type Result = { readonly itemId: string; readonly text: string };
...
export class UpdateChecklistItemUseCase {
  ...
  execute(input: Input): Result {
    ...
    const event = createChecklistItemUpdatedEvent({
    ...
    return { itemId: input.itemId, text: input.text };
  }
}
