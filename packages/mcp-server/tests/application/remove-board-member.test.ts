...
describe('RemoveBoardMemberUseCase', () => {
  ...
  let removeMember: RemoveBoardMemberUseCase;
  ...
  beforeEach(() => {
    ...
    removeMember = new RemoveBoardMemberUseCase(eventStore, readModel, projection);
  });

  it('removes a member from a board', () => {
    const { boardId } = createBoard.execute({ name: 'Board', actorId: OWNER });
    addMember.execute({ boardId, memberId: 'agent-2', actorId: OWNER });

    const result = removeMember.execute({ boardId, memberId: 'agent-2', actorId: OWNER });

    expect(result.members).not.toContain('agent-2');
    expect(readModel.findById(boardId)?.members).not.toContain('agent-2');
  });

  it('throws when board not found', () => {
    expect(() =>
      removeMember.execute({ boardId: 'missing', memberId: 'agent-2', actorId: OWNER }),
  ...
  it('throws when actor is not the owner', () => {
    ...
    expect(() =>
      removeMember.execute({ boardId, memberId: 'agent-2', actorId: 'agent-2' }),
    ).toThrow('Only the board owner');
  });

  it('appends BoardMemberRemoved event', () => {
    const { boardId } = createBoard.execute({ name: 'Board', actorId: OWNER });
    addMember.execute({ boardId, memberId: 'agent-2', actorId: OWNER });
    removeMember.execute({ boardId, memberId: 'agent-2', actorId: OWNER });

    const events = eventStore.getStream(boardId);
    expect(events[events.length - 1].type).toBe('BoardMemberRemoved');
  });
});
